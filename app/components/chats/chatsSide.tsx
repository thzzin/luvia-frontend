            ...
            {chats && chats.length > 0 ? (
                chats.map((chat) => {
                    ...
                    return (
                        <button 
                            key={chat.id} 
                            className="w-full text-left p-4 border-b flex justify-between items-center hover:bg-gray-100"
                            onClick={() => handleChatClick(chat.id)}
                        >
                            <div className="flex items-start justify-start md:mr-4">
                                <Avatar className="h-14 w-14 mr-4">
                                    <AvatarImage src={avatar || "https://via.placeholder.com/150"} />
                                    <AvatarFallback>{name?.charAt(0)}</AvatarFallback>
                                </Avatar>

                                <div className="md:ml-2 flex justify-between w-full">
                                    <div>
                                        <h2 className="font-semibold">
                                            {name || "Sem nome"} <strong className="text-gray-600 text-sm">{phone}</strong>
                                        </h2>
                                        <h2 className="ml-2 text-sm text-gray-600 truncate max-w-[50vw] md:max-w-[10vw]">{lastMessageContent}</h2>
                                    </div>
                                </div>
                            </div>
